 ...
 * @param <M>
 */
public class PrincipalVariationEngine<P extends Player<P>, M extends Move>
		implements Engine<P, M> {
	private final Board<P, M> board;

	private M[] bestMoves;

	public PrincipalVariationEngine(Board<P, M> board) {
		this.board = board;
	}
	...
	protected int computeBestMove(P player, int depth, int alpha, int beta) {
		...
		boolean principalVariationFound = false;
		while (moves.hasNext()) {
			M move = moves.next();
			make(move);
			...
			if (principalVariationFound) {
				score = -computeBestMove(player.opponent(), depth - 1,
						-alpha - 1, -alpha);
				...
				score = -computeBestMove(player.opponent(), depth - 1, -beta,
						-alpha);
			}
